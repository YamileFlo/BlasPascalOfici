        ...
        <div class="card-body" style="color: black;">
            <form method="post">
                <div class="form-group">
                    ...
                    // Obtener las asignaturas que ya están asignadas a otros docentes
                    $assigned_query = mysqli_query($conexion, "SELECT asignatura FROM asignar_docente_asignatura WHERE docente != {$_GET['iddoc']}");
                    $assigned_subjects = [];
                    while ($assigned = mysqli_fetch_assoc($assigned_query)) {
                        $assigned_subjects[] = $assigned['asignatura'];
                    }
                    $assigned_subjects_str = implode(",", $assigned_subjects);

                    // Consulta para mostrar solo las asignaturas no asignadas a otros docentes o asignadas a este docente actual
                    $query = mysqli_query($conexion, "SELECT rel.idrelgrado, au.idaula, au.seccion,
                        ...
                        WHERE rel.idrelgrado NOT IN ($assigned_subjects_str)
                        OR rel.idrelgrado IN (
                            SELECT asignatura FROM asignar_docente_asignatura WHERE docente = {$_GET['iddoc']}
                        )");
